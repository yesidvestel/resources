...
function GiveVehicle(source, vehicle)
    ...
    TriggerClientEvent('QBCore:Notify', src, 'Felicitaciones por su vehículo Startpack. ¡Consulte los garajes de su teléfono!', 'success')
		...
		if Player then
            ...
            if result then
                ...
                if args[2] then
                    ...
                    TriggerClientEvent('QBCore:Notify', src, "Se han configurado los metadatos del paquete de inicio", "primary")
                else
                    TriggerClientEvent('QBCore:Notify', src, "Proporcione el estado verdadero o falso", "error")
                end
            else
                TriggerClientEvent('QBCore:Notify', src, "Identificación de ciudadano no encontrada", "error")
            end
		end
	else
	...
	if args[1] then
		...
		if Player then
            ...
            if result then
                ...
                TriggerClientEvent('QBCore:Notify', src, "El estado del reclamo del paquete inicial es " .. status, "primary")
            else
                TriggerClientEvent('QBCore:Notify', src, "Identificación de ciudadano no encontrada", "error")
            end
		end
	else
        TriggerClientEvent('QBCore:Notify', src, "Por favor proporcione identificación ciudadana", "error")
	end
end, "admin")