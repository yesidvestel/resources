...
function download() {
	...
	formData.append(isUrl(Config.Webhook) ? "file" : "video", blob, "video.webm");
	let requestOptions = {
		method: 'POST',
		headers: myHeaders,
		body: formData,
	...
	if (!isUrl(webhook_url)) {
		webhook_url = `https://api.fivemanage.com/api/video?apiKey=${Config.Webhook}`
	}
	fetch(webhook_url, requestOptions)
		...
		.then(result => {
			...
			const url = result.attachments ? result.attachments[0].proxy_url : result.url
