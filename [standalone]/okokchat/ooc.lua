...
AddEventHandler('chat:do', function(id, name, message, time)
    ...
    if id2 == id1 then
        TriggerEvent('chat:addMessage', {
			template = '<div class="chat-message cloud"><i class="fab fa-dashcube"></i> <b><span style="color: #2aa9e0">[DO]{0}</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{2}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
			args = { name, message, time }
		})
